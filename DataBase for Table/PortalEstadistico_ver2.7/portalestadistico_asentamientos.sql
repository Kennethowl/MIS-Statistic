...
CREATE TABLE `asentamientos` (
  ...
  `Nombre_asentamientos` text,
  `ID_TipoAsent` int DEFAULT NULL,
  `ID_Servicios` int DEFAULT NULL,
  PRIMARY KEY (`ID_Asentamiento`),
  KEY `asentamientos_fbki1_idx` (`ID_TipoAsent`),
  KEY `asentamientos_fbki2_idx` (`ID_Servicios`),
  CONSTRAINT `asentamientos_fbki1` FOREIGN KEY (`ID_TipoAsent`) REFERENCES `tipoasentamiento` (`ID_Asentamiento`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `asentamientos_fbki2` FOREIGN KEY (`ID_Servicios`) REFERENCES `servicios` (`ID_Servicio`) ON DELETE CASCADE ON UPDATE CASCADE
